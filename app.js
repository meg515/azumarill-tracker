...
function getMergedCoreCards() {
  return MASTER_CORE_CARDS.map(card => {
    ...
    return {
      ...
      imageUrl: personal.imageUrl || card.imageUrl || null,
      imageSource: personal.imageUrl ? (personal.imageSource || "manual") : (card.imageSource || null),
    };
  });
}
...
async function init() {
  ...
  // Populate verified English card images in the background. This uses only
  // three Pokémon TCG API requests (one per Pokédex number), then matches
  // locally by species + set + printed card number. Saved matches live in
  // IndexedDB, so they do not need to be fetched again on later visits.
}
...
function variantBadgeInfo(card) {
  const raw = String(card.finishTarget || card.specialPrinting || "Standard").trim();
  const normalized = raw.toLowerCase();

  let kind = "standard";
  if (normalized.includes("reverse")) kind = "reverse";
  else if (normalized.includes("mirror")) kind = "mirror";
  else if (normalized.includes("1st") || normalized.includes("first edition")) kind = "first";
  else if (normalized.includes("unlimited")) kind = "unlimited";
  else if (normalized.includes("stamped") || card.isStamped) kind = "stamped";
  else if (normalized.includes("holo")) kind = "holo";
  else if (normalized.includes("promo") || card.isPromo) kind = "promo";
  else if (normalized.includes("deck")) kind = "deck";
  else if (normalized.includes("illustration") || normalized.includes("art rare")) kind = "art";

  return { label: raw.toUpperCase(), kind };
}
...
function cardTileHtml(card) {
  ...
  const variant = variantBadgeInfo(card);
  return `
    <div class="card-tile" data-stablekey="${card.stableKey}">
      <div class="card-tile-image">
        ...
        <span class="variant-badge variant-${variant.kind}">${esc(variant.label)}</span>
      </div>
...
// ---- Automatic English image population via Pokémon TCG API ----
// The unauthenticated API permits 1,000 requests/day and 30/minute. We make
// only three batch requests, one for each family Pokédex number, and never
// embed an API key in this public GitHub Pages app.
function normalizeSetName(value) {
  return String(value || "")
    ...
    .replace(/&/g, " and ")
    .replace(/[^a-z0-9]+/g, " ")
    .replace(/\b(expansion|set)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
}
function printedNumber(value) {
  return String(value || "").split("/")[0].trim().replace(/^0+(\d)/, "$1");
}
function confidentApiImageMatch(masterCard, apiCards) {
  const wantedSet = normalizeSetName(masterCard.product);
  const wantedNum = printedNumber(masterCard.cardNumber);
  const wantedSpecies = String(masterCard.species || "").toLowerCase();

  const candidates = apiCards.filter(apiCard => {
    const apiName = String(apiCard.name || "").toLowerCase();
    return apiName.includes(wantedSpecies) && printedNumber(apiCard.number) === wantedNum;
  });

  const exact = candidates.filter(c => normalizeSetName(c.set?.name) === wantedSet);
  if (exact.length === 1) return exact[0];

  // Accept a single containment match for minor naming differences such as
  // "... Promos" vs "... Black Star Promos". Never guess when ambiguous.
  const contained = candidates.filter(c => {
    const apiSet = normalizeSetName(c.set?.name);
    return apiSet && wantedSet && (apiSet.includes(wantedSet) || wantedSet.includes(apiSet));
  });
  return contained.length === 1 ? contained[0] : null;
}
async function autoPopulateEnglishImages() {
  if (state.autoImageLookupRunning) return;

  const englishMissing = getMergedCoreCards().filter(c => c.language === "English" && !c.imageUrl);
  if (!englishMissing.length) return;

  // A successful run is remembered for this dataset. Manual "Find image"
  // remains available for anything the conservative matcher leaves unresolved.
  const attemptKey = `azf-auto-images-${MASTER_DATASET_LABEL}-v1`;
  if (localStorage.getItem(attemptKey) === "complete") return;

  state.autoImageLookupRunning = true;
  try {
    const dexBySpecies = { Azurill: 298, Marill: 183, Azumarill: 184 };
    const apiBySpecies = {};

    for (const [species, dex] of Object.entries(dexBySpecies)) {
      const q = encodeURIComponent(`nationalPokedexNumbers:${dex}`);
      const url = `https://api.pokemontcg.io/v2/cards?q=${q}&pageSize=250&select=id,name,number,set,images`;
      const res = await fetch(url);
      if (!res.ok) throw new Error(`Pokémon TCG API ${res.status}`);
      const payload = await res.json();
      apiBySpecies[species] = payload.data || [];
    }

    let saved = 0;
    for (const card of englishMissing) {
      const match = confidentApiImageMatch(card, apiBySpecies[card.species] || []);
      const imageUrl = match?.images?.large || match?.images?.small;
      if (!imageUrl) continue;

      await dbSetOwnership(card.stableKey, {
        imageUrl,
        imageSource: "auto-matched",
      });
      saved++;
    }

    state.ownership = await dbGetAllOwnership();
    localStorage.setItem(attemptKey, "complete");
    if (saved) {
      refreshAllViews();
      if (state.currentTab === "collection") renderCollection();
      toast(`${saved} verified English card images added`);
    }
  } catch (err) {
    console.warn("Automatic English image lookup deferred:", err);
    // Do not mark the run complete. A later visit can retry automatically.
  } finally {
    state.autoImageLookupRunning = false;
  }
}
...
async function updateStorageInfo() {
  const privacyNote = `Your personal collection data is stored locally on this device. Most English card images are pre-mapped to static Pokémon TCG image URLs. The optional Find image tool may contact the Pokémon TCG API for unresolved English cards; your ownership, purchase, seller, price, and notes data are not sent with those lookups. Export a backup to keep your data safe.`;
  try {
